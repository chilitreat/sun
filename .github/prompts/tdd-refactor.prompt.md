...
description: 'TDD Refactorの設定作業を実行します。TDDのRefactorフェーズ（コード改善）を行い、テストが通る状態でコードをリファクタリングします。'
---

# TDD Refactor フェーズ（コード改善）

TDD の Refactor フェーズを実行します。
...
読み込み完了後、準備されたコンテキスト情報を基に Refactor フェーズ（コード改善）の作業を開始します。
...
Green フェーズで実装されたコードを以下の観点で改善してください。**テストは必ず通り続けること**が大前提です。
...
1. **メモファイル最終更新**: docs/implements/{{task_id}}/{feature_name}-memo.md ファイルの Refactor フェーズセクションと概要を更新
   - 改善内容、セキュリティレビュー結果、パフォーマンスレビュー結果を記録
   - 最終コード、品質評価を記録
   - 概要セクションの現在のフェーズを「完了」に更新
2. リファクタリング内容と設計改善を docs/implements/{{task_id}}/{feature_name}-refactor-phase.md に保存（既存ファイルがある場合は追記）
3. TODO ステータスを更新（Refactor フェーズ完了をマーク）
